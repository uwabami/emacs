...
** Debian パッケージがインストールされているならばそれを優先する
   ...
   Emacs に関連する Debian パッケージを幾つかメンテナンスしているので,
   可能な限り Debian パッケージを使うことにしています.
** [[https://github.com/conao3/leaf.el][leaf.el]] で Emacs のパッケージの導入と設定を行なう
   :PROPERTIES:
...
** 設定は [[http://orgmode.org/][Org mode]] で書きたい
   ...
   :END:
   以前こんなブログ記事を書きました→ [[http://uwabami.junkhub.org/log/20111213.html#p01][Emacs の設定ファイルを org で書く]]
...
* =exec-path-from-shell=: 環境変数の読み込み
  ...
  :END:
  shell (zsh) で設定した =PATH= などの環境変数を Emacs に引き継ぐために
...
* 言語の設定
  ...
  :END:
  最近の Emacs は locale から文字コードを自動判別するらしいので,
...
(leaf cp5022x
  ...
  (set-coding-system-priority 'utf-8 'euc-jp 'iso-2022-jp 'cp932)
   ...
   #+end_src
* 主に Emacs 本体, および同梱されている拡張に関する設定
  :PROPERTIES:
...
(leaf recentf
  ...
  #+end_src
* =all-the-icons-in-terminal=: ターミナルでも icon font を使いたい.
  :PROPERTIES:
...
** SOMEDAY East Asian Ambiguos 対応 [0/1]
   ...
   :END:
   East Asian Ambiguos を 2 文字幅にして, ついでに
   CJK 以外の East Asian Ambiguos と絵文字も 2 文字幅にするようにしています.
...
;;  )
  ...
  - [[http://syohex.hatenablog.com/entry/20111026/1319606395][cde を改良]]
  - [[https://qiita.com/__hage/items/2dd732b4dd68e124e8bd][cde と elscreen-separate-buffer-list の相性が悪い]]
  などを参照のこと.
  #+begin_src emacs-lisp :tangle init.el
...
* 日本語入力: =ddskk=
  ...
  ちなみに GTK が有効になっていると =gtk-immodule= なんかと衝突するので
...
(setq skk-jisx0201-mode-string "[_〓]")
...
*** 辞書の設定
    ...
    - はてなキーワード
    - [[http://matsucon.net/material/dic/][2 ちゃんねる顔文字辞書 MatsuCon]]
    ...
    顔文字に ; や が含まれている場合に, 適宜 quote する必要があるので
    以下のスクリプトで適当に変換.
...
(leaf ido
  ...
  `(;; prefix match 入力を先頭一致可能
    ...
    ;; run-with-timer に与える値なので, とりあえず長め (30 分) にしておく
...
** =lookup=: 電子辞書の検索
   ...
   :END:
   EPWING 化した辞書群を検索するために =lookup-el= ver. 1.4 系列を利用
   #+begin_src emacs-lisp :tangle init.el
(leaf lookup
  ...
  (lookup-search-agents
   . '((ndeb "/usr/local/share/dict/eijiro" :alias "英辞郎")
       ...
       (ndeb "/usr/local/share/dict/rikagaku5" :alias "理化学辞典 第 5 版")
       (ndeb "/usr/local/share/dict/koujien4" :alias "広辞苑 第 4 版")
       (ndeb "/usr/local/share/dict/wadai5" :alias "研究社 和英大辞典 第 5 版")
       (ndeb "/usr/local/share/dict/eidai6" :alias "研究社 英和大辞典 第 6 版")
...
**** cp5022x を使う
     ...
     [[http://d.hatena.ne.jp/kiwanami/20091103/1257243524][Wanderlust と文字コード]] も参照のこと.
     #+begin_src emacs-lisp :tangle init-wl.el
...
**** dispose, delete の設定
     ...
     :END:
     Gmail 用に%INBOX では削除を =wl-trash-folder= への移動ではなく, 「 delete 」に.
     #+begin_src emacs-lisp   :tangle init-wl.el
...
(setq elmo-msgdb-extra-fields
      '(
        ...
        "X-Google-Appengine-App-Id" ;; GAE の送信するメールの振り分け用
        "To"
        "Cc"
        "From"
        "Subject"
...
* 対応する括弧を見易く: =rainbow-delimiters=
  ...
  global に有効にするのは他の
  Major mode との衝突があるので止めた方が良い, らしい.
  #+begin_src emacs-lisp :tangle init.el
...
(leaf org-capture
  ...
  (setq org-capture-templates
   ...
   #+end_src
** Org と Google カレンダーの連携: =org-gcal=
   :PROPERTIES:
...
(leaf *orgmine
  ...
  (leaf orgmine
   ...
   #+end_src
** Org と Trello の連携: =org-trello=
   :PROPERTIES:
   :CUSTOM_ID: org0ea55d7d
   :END:
   ToDo 管理に Trello を使い始めたので, 設定
   #+BEGIN_SRC emacs-lisp
;; (leaf org-trello
;;   :if (and my:d:password-store
;;            (file-directory-p my:d:org))
...
* SOMEDAY 言語毎の補完: =company-mode=
  ...
  :END:
  まだうまく使いこなせていない.
  - [[https://qiita.com/wktkshn/items/3ac46671d1c242a59f7e][company-dabbrev で日本語を補完しない]]
  を有効にしてみたが, これで良いのかしら...?
  #+begin_src emacs-lisp :tangle init.el
(leaf company
  :ensure t
