...
(eval-and-compile
  ...
  (package-activate-all))
   #+end_src
** TODO [[https://github.com/conao3/leaf.el][leaf.el]] [1/1]
   :PROPERTIES:
...
(add-to-list 'package-vc-allow-build-commands 'hogehero)
   ...
   ついでに(どうやら?) =package-vc-install= で導入したパッケージは
   =package-selected-package= に登録されず,
   =package-auto-remove= を呼ぶと削除対象になる模様.
   #+begin_src emacs-lisp :tangle init.el
(leaf package-vc
...
(leaf eaw-halfwidth
  ...
  :hook (emacs-startup-hook . (lambda ()
                                (let ((load-file-message nil))
                                  (eaw-init)
                                  (global-auto-composition-mode 0))))
  )
   #+end_src
   とりあえず祟りが少ないので EAW は 1 文字幅にしているが,
...
;;;###autoload
  (defun my:setup-gui-frame (&optional frame)
    ...
    (let ((f (or frame (selected-frame))))
        ...
        ;; (set-frame-parameter f 'alpha '(95 . 90))
        ))
;;;###autoload
  (defun my:apply-font-eaw-console ()
    "EAW CONSOLE を適用する"
    (interactive)
    (set-face-attribute 'default nil :family "EAW CONSOLE" :height 135)
    )
;;;###autoload
  (defun my:apply-font-eaw-console-with-emoji ()
    "EAW CONSOLE と Noto Color Emoji を適用する"
    (interactive)
    (set-face-attribute 'default nil :family "EAW CONSOLE" :height 135)
    (let ((color-font (font-spec :family "Noto Color Emoji")))
      (set-fontset-font nil 'emoji color-font nil 'append)
      (set-fontset-font nil '(#x1F004 . #x1FFFD) color-font nil 'append))
    )
;;;###autoload
  (defun my:apply-font-plemol-jp ()
    "PlemolJP とカラー絵文字を適用する"
    (interactive)
    (set-face-attribute 'default nil :family "PlemolJP Console NF" :height 125)
    (let ((emoji-font (font-spec :family "Noto Color Emoji")))
      (set-fontset-font nil 'emoji emoji-font)
      (set-fontset-font nil '(#x1F004 . #x1FFFD) emoji-font nil 'append))
    )
  :config
  ...
  (window-setup-hook
   . (lambda ()
       (my:setup-gui-frame)
       (my:apply-font-eaw-console-with-emoji)))
...
** customize で行なっている雑多な設定など
   ...
   他にもイロイロと. 設定が増えてきたり複雑になったら分ける(かも).
   #+begin_src emacs-lisp :tangle init.el
(leaf cus-start
  :tag "builtin"
  ...
  (setq ;; setq か customize-set-variable か setop か, という話はあるものの.
   ;; 環境変数的なあれこれ -------------------------------------------------
   ;; 名前とメールアドレス
   user-full-name (concat (getenv "DEBFULLNAME"))
  ...
  ;; minibuffer も静かに: これは t だと shutup できない, という罠
   ...
   ;; tab 幅 8 ← org-element が tab幅 8 に強制してくる
   tab-width  8
   ;; RFC5322 風味
   fill-column  78
   ;; 折り返し無し
...
(leaf whitespace
  ...
  (defun my:configure-whitespace-for-buffer ()
    "Locally disable trailing whitespace cleanup if the file matches the exclude list."
    (when (buffer-file-name)
      (let ((patterns my:delete-trailing-whitespace-exclude-suffix)
        ...
        (when found
          ...
          (setq whitespace-style (delete 'trailing (append whitespace-style nil)))
          ))))
  :init
...
;;;###autoload
  ...
  (let* ((base-dir (expand-file-name my:d:org))
    ...
    (when (file-exists-p mstdn-current)
      (add-to-list 'my-agenda-list mstdn-current))
    ;; グローバル変数へ適用
...
;;  :advice ((:before org-gcal-fetch my:setup-org-gcal-secrets-lazy)
;;           (:before org-gcal-sync my:setup-org-gcal-secrets-lazy)
;;           (:before org-gcal-post-at-point my:setup-org-gcal-secrets-lazy))
  :preface
  (setq org-gcal-dir (expand-file-name "org-gcal" my:d:tmp))
  (unless (file-directory-p org-gcal-dir)
    (make-directory org-gcal-dir))
;; ;;;###autoload
;; (defun my:setup-org-gcal-secrets-lazy (&rest _args)
;;   "org-gcal 関連の変数をループ処理で plstore から読み込む"
;;   (interactive)
;;   (unless (and (boundp 'org-gcal-client-id)
;;                org-gcal-client-id)
;;     (dolist (var '(org-gcal-client-id
;;                    org-gcal-client-secret
;;                    org-gcal-file-alist))
;;       (set var (eval `(leaf-handler-auth org-gcal ,var leaf-default-plstore))))))
  :init
  (leaf oauth2-auto
    ;; 🤔
    :init
  ...
  (setq org-gcal-dir (expand-file-name "org-gcal" my:d:tmp) ;; preface で設定済?
    ...
    (org-gcal-reload-client-id-secret)
    (message "org-gcal: Secrets configured.")
  )
   #+end_src
...
(leaf modus-themes
  ...
  (defun my:modus-themes-custom-faces (&rest _args)
    (modus-themes-with-colors
      (custom-set-faces
       ;; vertico
       `(vertico-current                                  ((t :inherit hl-line :background unspecified )))
       ;; wanderlust
