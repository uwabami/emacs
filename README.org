...
* LSP: =eglot=
  ...
  :END:
  Language Protocol Server のクライアントとしては =lsp-mode,lsp-ui= もあるが,
  [[https://github.com/joaotavora/eglot][eglot]]の方が依存が少なくて軽量なので好み.
  #+begin_src emacs-lisp :tangle init.el
(leaf eglot
...
** modeline: =moody=
   :PROPERTIES:
   :CUSTOM_ID: orgca564786
   :END:
   以前は powerline を使っていたけれど, [[https://github.com/tarsius/moody][moody]] に変更してみたり.
   #+begin_src emacs-lisp
(leaf moody
  :ensure t
  :init
  ...
  (defun my:modeline-status ()
    "Return SKK indicator and buffer status"
    (my:modeline-ddskk)
    (format "%s %s (%s) "
            ;; skk indicator
            ...
            (let ((read-only  buffer-read-only)
              ...
              (cond
               ...
               (t (if (display-graphic-p)
                    ...
                    my:modeline-tty-rw-symbol))))
            ;; major mode icon
            (my:major-mode-icon major-mode))
    )
  :advice
  `((:override skk-setup-modeline my:skk-setup-modeline)
    )
  :config
  ;;
  ...
  (moody-replace-element 'mode-line-position '(""))
  ;; (moody-replace-element 'mode-line-misc-info '(""))
  (moody-replace-element 'mode-line-modes '(""))
  ...
  (moody-replace-element 'mode-line-misc-info
   ...
   #+end_src
** minor mode の非表示: =minions=
   :PROPERTIES:
   :CUSTOM_ID: orgb9974e5f
   :END:
   #+begin_src emacs-lisp :tangle init.el
(leaf minions-mode
  :disabled t
