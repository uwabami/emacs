...
;;;###autoload
  ...
  ;; ---------------------------------------------------------------------------
  ;; corfu-で始まるかで動作が変わるところがある。
  ;; - savex C-M-i RETと素早く押したときに、RETがcorfu-で始まるコマンドだと
  ;;   updateが走り正常に動作するが、my-corfu-だとそうならない。
;;;###autoload
  (defun corfu-my:insert-RET ()
    (interactive)
    (corfu-insert)
    (call-interactively 'newline)) ;; インタラクティブじゃないとインデントされなかったり
  ;; ---------------------------------------------------------------------------
;;;###autoload
...
;; ---------------------------------------------------------------------------
  :advice ((:around corfu--auto-complete-deferred
           ...
           (:around corfu--make-frame
                    my:corfu--make-frame:around)
           )
  ;; -------------------------------------------------------------------------
  :init
  ;; -------------------------------------------------------------------------
