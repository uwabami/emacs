...
(leaf *ido-recentf-open
  ...
  (defun ido-recentf-open ()
    ...
    (let ((files (mapcar (lambda (f)
                         ...
                         recentf-list)))
      (let ((selected (ido-completing-read "Recent Files: " (mapcar #'car files))))
...
(leaf consult
  ...
  (defun my:consult-recent-file ()
    ...
    (interactive)
    (setq-local marginalia-mode -1)
    (find-file
     (consult--read
      (or (mapcar
           (lambda (f)
             (cons (my:shorten-file-path f 70) f)) recentf-list)
  ...
  (setq consult-config `((my:consult-recent-file :preview-key nil)))
  )
   #+end_src
