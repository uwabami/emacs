...
%.el: %.org
	$(EMACS) -Q --batch --eval \
	   "(progn \
         (add-to-list 'load-path (concat user-emacs-directory \"share/org-mode/lisp\")) \
	     (require 'ob) \
	     (require 'ob-tangle) \
...
tmp/.permission-stamp: tmp/.skkdic-stamp
	chmod 700 tmp
	touch $@
tmp/.skkdic-stamp: tmp/.org-mode-stamp
	@echo "setup ddskkdic"
	...
	touch $@
tmp/.org-mode-stamp:
	@echo "byte-compile org-mode"
	$(MAKE) compile -C share/org-mode
	touch $@
