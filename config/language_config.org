...
# $Lastupdate: 2013/01/24 02:38:22$
# License: Expat
#
#+TITLE: 言語の設定
...
* East Asian Ambiguos 対応
  CJK 以外の East Asian Ambiguos も2文字幅にする
  #+BEGIN_SRC emacs-lisp
  ...
  #+END_SRC
* OSの違いに起因する条件分岐
  Mac と Linux では同じ Unicode でも正規化が異なる.
  (具体的には Mac のファイルシステムである HFS+ ではUnicode の正規化が異なる).

  日本語のファイル名を扱うことは滅多にないものの,
  たまに祟りがあるのでそれを回避するための設定をしておく.
