...
(defun my:event-apply-control-meta-modifiers (ignore-prompt)
  ...
  (vector
   (event-apply-modifier (event-apply-modifier (read-event)
                         ...
                         'meta 27 "M-")))
;; -------------------------------------------------------------------
(keymap-set function-key-map "C-x @" 'my:event-apply-control-meta-modifiers)
   #+end_src
...
;; 何か意味のあるキーが良いのだけれどなぁ
(keymap-global-set "C-c l" #'my:fill-line-with-char)
   #+end_src
...
** TODO East Asian Ambiguos 対応 [0/2]
   ...
   #+begin_src emacs-lisp :tangle init.el
;;(leaf eaw-console
;;  :vc (:url "git@github.com:hamano/locale-eaw" :lisp-dir "dist" :main-file "eaw-console.el")
;;  :commands eaw-init
;;  :init
...
;;  )
(leaf eaw-custom
 :vc (:url "git@github.com:uwabami/locale-eaw-from-fontset" :branch "foot")
 :init
 :hook (emacs-startup-hook . (lambda ()
                               (eaw-init)
                               (global-auto-composition-mode 0)))
...
(defun my:setup-gui-frame (&optional frame)
  ...
  (let ((f (or frame (selected-frame))))
    (when (display-graphic-p f)
      ...
      (set-face-attribute 'default nil :family "Victor Mono" :weight 'medium :height 120)
      ;; (set-face-attribute 'default nil :family "EAW CONSOLE" :weight 'medium :height 120)
      ;; フォントセットの設定. 対象フレーム (第4引数) を指定せずグローバルに適用する.
      (set-fontset-font nil 'unicode (font-spec :family "Victor Mono" :weight 'medium))
      ;; (set-fontset-font nil 'unicode (font-spec :family "EAW CONSOLE" :weight 'medium))
      (dolist (code '(katakana-jisx0201
        ...
        ;; (set-fontset-font nil code (font-spec :family "EAW CONSOLE") nil 'append)
        ...
        (set-fontset-font nil code (font-spec :family "PlemolJP Console") nil 'append)
        )
      (eval-when-compile (require 'eaw-custom nil :noerror))
      ;; (eval-when-compile (require 'eaw-console nil :noerror))
      (dolist (target code-wide)
        ...
        ;; (set-fontset-font nil target (font-spec :family "EAW CONSOLE") nil 'append)
        (set-fontset-font nil target (font-spec :family "PlemolJP Console") nil 'append)
        )
      (dolist (target code-half)
        ...
        ;; (set-fontset-font nil target (font-spec :family "EAW CONSOLE"))
        )
      ))
  )
;; ---------------------------------------------------------------------------
...
(leaf nerd-icons
  ...
  (setq nerd-icons-font-family "EAW CONSOLE")
  :defer-config
...
(leaf tab-bar-mode
  ...
  (keymap-set global-map "C-o" 'my:ctrl-o-prefix)
  (keymap-set my:ctrl-o-map "c"   'tab-new)
  (keymap-set my:ctrl-o-map "C-c" 'tab-new)
  (keymap-set my:ctrl-o-map "k"   'tab-close)
  (keymap-set my:ctrl-o-map "C-k" 'tab-close)
  (keymap-set my:ctrl-o-map "n"   'tab-next)
  (keymap-set my:ctrl-o-map "C-n" 'tab-next)
  (keymap-set my:ctrl-o-map "p"   'tab-previous)
  (keymap-set my:ctrl-o-map "C-p" 'tab-previous)
;;;###autoload
...
;;        )))
  ...
  (f90-ts-nav-procedure-face               . '((t :inherit font-lock-function-name-face :weight bold)))
  (f90-ts-nav-type-face                    . '((t :inherit font-lock-type-face :weight bold)))
  (f90-ts-nav-module-face                  . '((t :inherit font-lock-function-name-face :weight bold)))
  (f90-ts-nav-variable-face                . '((t :inherit default)))
  ...
  (setq f90-ts-indent-toplevel             2
        ...
        f90-ts-indent-lists-line-2         'continued-line
        f90-ts-indent-lists-line-3         'continued-line
        f90-ts-indent-paren-default        1
        f90-ts-indent-paren-close          0
        f90-ts-indent-expr-assign-default  2
        f90-ts-indent-expr-assign-assoc-op 0
        f90-ts-indent-declaration          3
        f90-ts-leading-ampersand-style     '(indent . 2)
        ...
        f90-ts-leading-ampersand           t
        f90-ts-menu-show-navigate          t
        ...
        f90-ts-extra-comment-prefixes '("! " "!$omp " "!$acc " "!!!" "!> " "!< ")
        ...
        ;; f90-ts-nav-buffer-auto-sync        t
        ;; f90-ts-nav-buffer-idle-delay       1.0
        ;; f90-ts-nav-buffer-width            40
...
(leaf wl
  ...
  (if (fboundp 'define-mail-user-agent)
      (define-mail-user-agent
        ...
        'mail-send-hook))
  ;; ---------------------------------------------------------------------------
  (defconst my:d:wl-cache-directory (expand-file-name "~/.cache/wl"))
...
*** キーバインド関連
    ...
    #+begin_src emacs-lisp :tangle init-wl.el
(keymap-set wl-draft-mode-map "C-c C-j" 'browse-url-at-point)
    #+end_src
    =M-u= で unread にする
    #+begin_src emacs-lisp :tangle init-wl.el
(keymap-set wl-summary-mode-map "M-u" 'wl-summary-mark-as-unread)
    #+end_src
    =i= で sync <- Mew 風
    #+begin_src emacs-lisp :tangle init-wl.el
(keymap-set wl-summary-mode-map "i" 'wl-summary-sync-update)
    #+end_src
    =C-o= は tabbar で使う
    #+begin_src emacs-lisp :tangle init-wl.el
(keymap-set wl-summary-mode-map "C-o" nil)
    #+end_src
    =M-o= で =auto-refile=  (Mew 風)
    #+begin_src emacs-lisp :tangle init-wl.el
(keymap-set wl-summary-mode-map "M-o" 'wl-summary-auto-refile)
    #+end_src
...
(keymap-set wl-summary-mode-map "=" 'my:wl-summary-jump-to-referer-message)
    #+end_src
...
(leaf c-sig
  ...
  :init
  (setq sig-alist-file (expand-file-name "~/.signature.alist"))
  :config
...
(keymap-set wl-draft-mode-map "C-c C-i" 'insert-signature-eref)
(keymap-set wl-draft-mode-map "C-c <tab>" 'insert-signature-eref)
     #+end_src
*** GPG 署名
    :PROPERTIES:
...
;;;###autoload
  ...
  ;; -------------------------------------------------------------------------
  :config
  (keymap-global-set "C-c C-j" 'browse-url-at-point)
  )
   #+end_src
** Org-capture: メモ取り
   :PROPERTIES:
...
(leaf ox-jekyll
  :if (file-directory-p "~/Public/cc-env")
  :vc (:url "git@github.com:uwabami/ox-jekyll")
  )
(leaf ox-html
  :after ox
  :init
...
;;   :require t
;;   )
;; (keymap-global-set "C-j" #'mozc-temp-convert)
;;(leaf emacs
;;  :preface
