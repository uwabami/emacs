...
(startup-redirect-eln-cache
 (expand-file-name "~/.cache/emacs/eln-cache/"))
   ...
   #+begin_src emacs-lisp :tangle (if (boundp 'native-comp-async-jobs-number) "early-init.el" "no")
...
(leaf *gui
  ...
  :preface
;;;###autoload
  (defun my:setup-gui-fonts (default-font-name
                             default-font-height
                             cjk-font-name
                             cjk-font-scale
                             emoji-font-name
...
;;;###autoload
  (defun my:apply-font-Moralerspace-Argon ()
    (interactive)
    (my:setup-gui-fonts "Moralerspace Argon HW" 130
                        "Moralerspace Argon HW" 1.0
                        "Moto Color Emoji for PlemolJP" 1.0
                        ))
;;;###autoload
  (defun my:apply-font-PlemolJP-Console ()
    (interactive)
    (my:setup-gui-fonts "PlemolJP Console NF" 120
                        "PlemolJP Console NF" 1.0
                        "Moto Color Emoji for PlemolJP" 1.1
                        ))
;;;###autoload
  (defun my:apply-font-custom ()
    (interactive)
    (my:setup-gui-fonts "Victor Mono" 135
                        "PlemolJP Console NF" 1.0
                        "Moto Color Emoji for PlemolJP" 1.0
                         ))
;; ;;;###autoload
;;   (defun my:apply-font-EAW-CONSOLE ();
;     (interactive)
;;     (my:setup-gui-fonts "EAW CONSOLE" 135
;;                         "EAW CONSOLE" 1.0
;;                         "EAW CONSOLE" 1.0
;;                         ))
;;;###autoload
  (defun my:setup-gui-frame (&optional frame)
    ...
    (let ((f (or frame (selected-frame))))
      (set-frame-parameter f 'reverse t)
        ;; もの凄く重くなるので, 一旦保留. 透明度, 不透明度の順
      (set-frame-parameter f 'alpha '(90 . 70))
      ))
  :config
  ...
  ((window-setup-hook
    . (lambda ()
        (my:setup-gui-frame)
        (my:apply-font-PlemolJP-Console))
    )
...
(leaf cus-start
  :tag "builtin"
  ;; < for show-paren syntax highlight
  :emacs> "28"
  :init
...
;;;###autoload
  ...
  (setq treesit-language-source-alist
   ...
   #+end_src
** =f90-ts-mode=: fortran の色付けを tree-sitter で
   :PROPERTIES:
...
;;;###autoload
  ...
  (setopt f90-ts-indent-toplevel 2
