...
# $Lastupdate: 2013/01/02 19:53:31$
# License: Expat.
#
   ...
   #+BEGIN_SRC emacs-lisp
     (eval-when-compile
       (add-to-list 'load-path
                    (concat user-emacs-directory "el-get/elscreen-knu-branch")))
     (require 'elscreen-wl)
   #+END_SRC
** cp5022x を使う
...
* 添付ファイルの扱い
  ...
  同じエントリがあると, 動作が微妙になる
  - [[http://comments.gmane.org/gmane.mail.wanderlust.general.japanese/8618]]
...
* GPG 署名
  ...
  #+END_SRC
* 検索
  最近は手元で dovecot を上げて, imap search で十分なので,
  以下の設定はしていません.
