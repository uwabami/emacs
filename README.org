...
(defun my:setup-gui-frame (&optional frame)
  ...
  (let ((f (or frame (selected-frame))))
    (when (display-graphic-p f)
      ...
      (set-face-attribute 'default nil :family "Iosevka Term" :weight 'light :height 130)
      ;; フォントセットの設定. 対象フレーム (第4引数) を指定せずグローバルに適用する.
      (set-fontset-font nil 'unicode (font-spec :family "Iosevka Term" :weight 'light))
