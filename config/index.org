...
# $Lastupdate: 2013/01/02 02:33:40$
# License: Expat
#
#+TITLE: Emacsの基本設定
...
** cp5022xの利用
   ...
   #+BEGIN_SRC emacs-lisp
     (when (require 'cp5022x nil 'noerror)
       (bundle 'cp5022x))
   #+END_SRC
* 機種依存しない(ハズ)の雑多な設定
  Emacs そのものの設定集.
...
** scratch を殺さない. 消したら再生成
   ...
   #+END_SRC
** 履歴ファイル管理
   undo-hist と recentf-ext について, あとで書くこと.
...
* MUA: [[https://github.com/wanderlust/wanderlust][Wanderlust]]
  ...
  #+BEGIN_SRC emacs-lisp
        ...
        (unless (locate-library "rail")
          (bundle rail
            (eval-after-load-compile 'rail
              (setq rail-emulate-genjis t))))
        (unless (locate-library "google-contacts-wl")
          (bundle google-contacts-wl
            (eval-after-load-compile 'google-contacts-wl
...
* [[http://emacswiki.org/emacs/auto-async-byte-compile.el][auto-async-byte-compile.el]]
  #+BEGIN_SRC emacs-lisp
    (bundle 'auto-async-byte-compile
      (eval-after-load-compile 'auto-async-byte-compile
...
* [[http://d.hatena.ne.jp/kiwanami/20110221/1298293727][パスワード管理ツール id-manager.el]]
  ...
  #+BEGIN_SRC emacs-lisp
    (bundle id-manager
      (eval-after-load-compile 'id-manager
...
* [[file:org-mode_config.org][Org-mode]]
  TeX/Beamer への export, org-capture の設定をしている
  #+BEGIN_SRC emacs-lisp
    (when (file-exists-p
...
* [[http://howm.sourceforge.jp/index-j.html][Howm]] 関連
  メモは howm + org-Capture で取って, Dropbox に保存してます.
  無いと途方に暮れる...
  #+BEGIN_SRC emacs-lisp
    (when (file-exists-p
           (expand-file-name (concat (getenv "HOME") "/Dropbox/org")))
...
* [[file:anything_config.org][Anything]]
  Debianでは[[http://packages.qa.debian.org/a/anything-el.html][anything-el]] [fn:10] としてパッケージ化されているので
  #+BEGIN_EXAMPLE
  $ apt-get install anything-el
  #+END_EXAMPLE
  で OK.

  パッケージが使えない場合には bundle ことにする.
  #+BEGIN_SRC emacs-lisp
    (unless (locate-library "anything")
      (bundle anything))
    (unless (locate-library "descbinds-anything")
      (bundle descbinds-anything
        (eval-after-load-compile 'descbinds-anything
