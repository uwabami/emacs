...
;; -*- lexical-binding: nil -*-
  #+end_src
* Byte-Compile 時の Common Lisp の読み込み, JIT(>= 28)
  :PROPERTIES:
...
(leaf vertico
      ...
      (save-match-data
        (when (search-backward-regexp sep nil t)
          ...
          (filter-buffer-substring (point)
                                   ...
                                   #'delete)))))
  ;;
  ;; (defun my:vertico--compare-considering-input (x y)
  ...
  ;;       (`(,match . ,_) match))))
  ;; (defun my:vertico-sort-considering-input (candidates)
  ;;   (sort candidates #'my:vertico--compare-considering-input))
  :advice
 ...
 `((vertico-current
  ...
  `((vertico-count . 9)
    ...
    ;;  . 'my:vertico-sort-considering-input)
    )
  :config
...
(leaf ob-latex
  :if (file-exists-p "/usr/bin/uplatex")
   ...
   #+end_src
** Org-roam, Org-roam-dailies: リンク付きメモ
   :PROPERTIES:
...
(leaf org-roam-dailies
  ...
  `(
   ...
   #+end_src
** =vulpea=: Org-roam, Agenda を繋ぐ
   :PROPERTIES:
   :CUSTOM_ID: org37d0b758
   :END:
   以下がとても参考になる.
   ...
   今のところ, 上記 5 番の  [[https://d12frosted.io/posts/2021-01-16-task-management-with-roam-vol5.html][Dynamic and fast agenda]] について設定している感じ.
   #+begin_src emacs-lisp
(leaf vulpea
  :if (and (file-directory-p my:d:org)
           (and (executable-find "rg")
...
;; @see https://d12frosted.io/posts/2020-06-24-task-management-with-roam-vol2.html
  ...
  (setq org-agenda-span 'week
        ...
        '(
          ("n" "agenda and all TODO list"
           (
            ;; (agenda . " %i %(my:vulpea-agenda-category 22)%?-12t% s")
            (agenda ""
                    ((org-agenda-ndays 1)
                     (org-agenda-entry-types '(:timestamp :sexp))))
            (todo "TODO"
                  ;; ((org-agenda-prefix-format " %i %(my:vulpea-agenda-category 22)%?-12t% s"))
...
(leaf modus-themes
  ...
  ((window-setup-hook . my:set-background)
   ...
   ;; (emacs-startup-hook . modus-themes-load-vivdendi)
   )
  ;; :hook
  ;; `(;; (window-setup-hook . modus-themes-load-vivendi)
  ;;   ;; (tty-setup-hook . modus-themes-load-vivendi)
  ;;   )
  :custom
  `(
