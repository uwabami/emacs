...
* 補完: =ido=, =flx-ido=
  ...
  find-file は helm-find-file より ido + flx-ido + ido-grid の方が
  好みなんだが，どうしようかなぁ...
** =ido= の設定
   #+BEGIN_SRC emacs-lisp
(use-package ido
  ;; 補完は基本的に helm に投げることに.
...
** helm 本体
  #+BEGIN_SRC emacs-lisp
    (use-package helm
      :ensure t
      :diminish (helm-migemo-mode helm-mode)
      :bind (("M-x"     . helm-M-x)
      ...
      ;; minibuffer で C-f で default の find-file に移行
      ;; write-file や insert-file なんかで必須.
      (defun my:default-ff (&optional no-op)
        "execute default find-file"
        (interactive)
        (call-interactively 'find-file))
      (setq helm-type-file-actios
            (append helm-type-file-actions
                    '(("Default Find File" . my:default-ff))))
      (with-eval-after-load "helm"
        (bind-keys :map helm-map
                   ("C-f" . (lambda () (interactive)
                              (helm-quit-and-execute-action 'my:default-ff)))))
      ;; helm と migemo の設定
