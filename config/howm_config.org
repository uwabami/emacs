...
# $Lastupdate: 2012/10/06 03:37:53$
# License: Expat
#
#+TITLE: howm の設定
...
* インストール
  無いと途方に暮れるので,
  環境に howm と elscreen-howm が無い場合には el-get しています.
  elscreen-howmも同様.
  ただ, 手元の el-get では tar.gz の展開に失敗するので
  Debian パッケージの Git リポジトリから取得するようにしています.
  #+BEGIN_SRC emacs-lisp
    (unless (locate-library "howm")
      (el-get 'sync 'howm-alioth))
    (unless (locate-library "elscreen-howm")
...
* ディレクトリの設定
  ...
  #+BEGIN_SRC emacs-lisp
    (when (file-exists-p
      ...
      (setq howm-keyword-file "~//Dropbox/org/.howm-keys")
      (setq howm-history-file "~//Dropbox/org/.howm-history")
      (setq howm-menu-file "~/Dropbox/org/.howm-menu"))
