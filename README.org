...
(leaf locale-eaw-emoji
  ...
  `(;;(emacs-startup-hook . eaw-and-emoji-fullwidth)
    ...
    (emacs-startup-hook . eaw-halfwidth)
   )
  )
   #+end_src
...
(leaf *gui
  ...
  (set-face-attribute 'default nil
                      ...
                      :height 135)
  ;; ≒ が全角幅なので…
  (set-fontset-font nil #x2252 (font-spec :family "PlemolJP Console NF"))
  ;; emoji の幅が合わない⇒ Twitter Color Emoji は monospace?
  (set-fontset-font nil '(#x1F004 . #x1FFFD) (font-spec :family "Twitter Color Emoji"))
  ;;
  ;; Variable pitch が嫌いです.
  (dolist (face
           '(fixed-pitch
             fixed-pitch-serif
             variable-pitch
    ...
    ;; まだありそう...
    (set-face-attribute face nil :family 'unspecified :height 'unspecified :inherit 'default))
  :hook
...
*** アイコンフォント: =nerd-icons=
   ...
   :END:
   以前は all-the-icons を利用していた.
   - Nerd Fonts の ver.2 まではアイコンが
     PUA をはみ出していて異体字の表示に支障があったのだけれど
     ver.3 からこれが解消された
...
(leaf nerd-icons
  ...
  :custom
  `((nerd-icons-font-family    . "UDEV Gothic NF")
...
** SEMIの設定
   :PROPERTIES:
   :CUSTOM_ID: org88726f1b
   :END:
   Wanderlust が本文の表示に使う SEMI の設定など
   #+begin_src emacs-lisp :tangle init.el
(leaf semi
  :init
  (leaf mime-view
...
(leaf wl
  ...
  `((elmo-msgdb-directory . my:d:wl-cache-directory)
    ...
    ;; From に応じて送受信サーバを切り替えるために. → hook も参照
    (wl-template-visible-select . t)
    )
...
(leaf org-gcal
  ...
  :defvar
  (org-gcal-client-id org-gcal-client-secret org-gcal-file-alist)
  :pl-setq
  (org-gcal-client-id org-gcal-client-secret org-gcal-file-alist)
  :ensure t
  ;; :el-get (org-gcal
