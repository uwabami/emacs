...
** Org-roam と Org-agenda
   :PROPERTIES:
   :CUSTOM_ID: orgb9dc2ae0
   :END:
   org-roam で db に query 投げて,
   =org-agenda-files= を更新するように.
   元ネタ: [[https://d12frosted.io/posts/2021-01-16-task-management-with-roam-vol5.html][Task management with org-roam Vol. 5: Dynamic and fast agenda]]
   #+begin_src emacs-lisp
(leaf vulpea
  :if (and (file-directory-p my:d:org)
           (and (executable-find "rg")
...
** 文献参照
   :PROPERTIES:
   :CUSTOM_ID: org070567c3
   :END:
   試行錯誤中...
*** bibtex-completion
    :PROPERTIES:
    :CUSTOM_ID: org9d970700
    :END:
    #+begin_src emacs-lisp
(leaf bibtex-completion
  :if (and (and (executable-find "uplatex")
                (executable-find "latexmk"))
           (file-directory-p my:d:org))
  :ensure t
  :custom
  `((bibtex-completion-pdf-symbol   . "")
    ...
    (bibtex-completion-bibliography
     . ,(expand-file-name "references.bib" my:d:nextcloud))
    (bibtex-completion-library-path
     . ,(expand-file-name "Zotero" my:d:nextcloud))
    (bibtex-completion-notes-path
     . ,(expand-file-name "INBOX.org" my:d:org))
    ;; Debian 11 から /usr/bin/open が alternatives で提供される様になった
    ;; 実態は xdg-open だが、macOS と条件分岐しなくて非常に楽に.
...
*** consult-bibtex
    :PROPERTIES:
    :CUSTOM_ID: orgab568a64
    :END:
    #+begin_src emacs-lisp
(leaf consult-bibtex
  :if (file-directory-p my:d:org)
  :el-get  mohkale/consult-bibtex
  )
    #+end_src
*** org-ref
    :PROPERTIES:
    :CUSTOM_ID: orge3334680
    :END:
    #+begin_src emacs-lisp
(leaf org-ref
  :if (file-directory-p my:d:org)
  :ensure t
  :custom
  `( ;; same as bibtex-completion
    (org-ref-default-bibliography
     . ,(expand-file-name "references.bib" my:d:nextcloud))
    (org-ref-bibliography-nots
     . ,(expand-file-name "INBOX.org" my:d:org))
    (org-ref-pdf-directory
     . ,(expand-file-name "Zotero" my:d:nextcloud))
    )
  :hook
  (org-ref-clean-bibtex-entry-hook . org-ref-replace-nonascii)
  )
    #+end_src
* Autoconf
  :PROPERTIES:
...
(leaf modus-themes
  ...
  (defun my:modus-themes-custom-faces ()
    (modus-themes-with-colors
      (custom-set-faces
       ...
       `(mode-line-highlight ((,class :foreground "#aaaaaa" :background "#4C4C4C")))
       ;;
       ;; だいぶ弄ってるなぁ….
       ;;
       ...
       `(font-lock-string-face               ((,class ,@(modus-themes--syntax-string
                                                         ...
                                                         green-intense green-intense))))
       `(font-lock-builtin-face              ((,class :inherit nil ;; modus-themes-bold
                                                      ,@(modus-themes--syntax-extra
                                                         blue-alt blue-alt-faint
                                                         blue-alt blue-alt-faint))))
       `(font-lock-constant-face             ((,class :inherit modus-themes-bold
                                                      ,@(modus-themes--syntax-extra
       ...
       `(font-lock-keyword-face              ((,class :inherit modus-themes-bold
                                                      ,@(modus-themes--syntax-extra
                                                         ...
                                                         red-alt-other red-alt-other-faint))))
       `(font-lock-function-name-face        ((,class :inherit modus-themes-bold
                                                      ,@(modus-themes--syntax-extra
                                                         purple-intense purple-intense
                                                         purple-intense purple-intense))))
  ...
  `(
    ...
    (modus-themes-completions . 'moderate)  ;; should be nil?
    (modus-themes-mail-citations . nil) ;; → wanderlust で設定
