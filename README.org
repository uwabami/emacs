...
(require 'bind-keys)
   #+END_SRC
   で良い，とあるのだけれど font-lock が効かなくなる様なので
   #+BEGIN_SRC emacs-lisp
(require 'use-package)
   #+END_SRC
   としている．
** pallet
   ...
   以前は「vcs から持ってくる」といった事ができなかったのだけれど，
   最近対応した様で，(ようやく)安心して使えるようになった．
   #+BEGIN_SRC emacs-lisp
(use-package pallet
  :config
  (pallet-mode t))
...
(use-package helm
  ...
  (advice-add 'helm-ff-kill-or-find-buffer-fname
              :around (lambda (orig-fn candidate)
...
(use-package orgmine
  ...
  :init
  ;; 幾つかの redmine で未だに textlie を使っているので...．
