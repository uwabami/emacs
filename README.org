...
*** TODO 起動後に GC Threshold を既定値に戻す [/]
    :PROPERTIES:
...
(add-hook 'emacs-startup-hook
    ...
    #+end_src
    - [ ] gcmh は無駄にタイマーを増やし過ぎて, 自分が増やしたタイマーで GC を走らせている, という指摘もある.
** Package初期化処理
   :PROPERTIES:
...
(leaf package-vc
  ...
  :custom `((package-vc-allow-build-commands . nil)
...
* =gcmh=: 賢い garbage-collection
  :PROPERTIES:
  :ID:       cca3e8df-79f9-4c60-a803-07ff641b0ef7
  :CUSTOM_ID: org414ff0d8
  :END:
  割と速い段階で設定しておきたくてここに書いたけれど,
  実際には =emacs-startup-hook= で起動するので, 最後辺りで有効になる.
...
(leaf gcmh
  ...
  :blackout t
  :init
  (setq gcmh-idle-delay 'auto  ; アイドル判定を自動調整
        gcmh-auto-idle-delay-factor 10
        gcmh-high-cons-threshold (* 128 1024 1024)) ; 入力中は 128MB まで許容
