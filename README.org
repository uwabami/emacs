...
** TODO East Asian Ambiguos 対応 [0/1]
   ...
   → 最近は foot を使っているので, 全て halfwidth にしました.
   なお, 環境変数でいわゆる set-language-environment "Japanese" と判定されると, EAW は 2文字幅に設定されてしまいます.
   これを強制的に 1 に戻す必要があります.
   #+begin_src emacs-lisp :tangle init.el
...
;;                                 ))
;;   )
;; (leaf glyphless-display-mode
;;   ;; :init (setq glyphless-char-display 'zero-width)
;;   :hook (emacs-startup-hook . glyphless-display-mode)
;;   )
(leaf eaw-single
  :commands eaw-init
  :vc (:url "https://github.com/uwabami/locale-eaw" :lisp-dir "dist")
  :hook (emacs-startup-hook . (lambda ()
                                (let ((load-file-message nil))
                                  ;; (load "eaw-console" nil t)
                                  (eaw-init)
                                  (global-auto-composition-mode 0))))
  )
   #+end_src
   - [ ] リテラシーの低い人からのメールで丸囲み数字が良く表われてシンドい(見切れる).
     - 適宜マッピングする?
     - もしくは1文字幅でも見易いフォントを使う. 現状は IBM Plex 関連?
** フォント
  :PROPERTIES:
