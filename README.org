...
* はじめに
  ...
  [[https://travis-ci.org/uwabami/emacs.svg]]
...
(package-install pkg-install)
   ...
   #+END_SRC
   とか．実際は [[file:Makefile][Makefile]] 参照．
...
(org-export-define-derived-backend 'jekyll 'html
  ...
  :filters-alist
  '((:filter-final-output . org-jekyll--escape-liquid))
  )
;; template
(defun org-jekyll-template (contents info)
  "Return complete document string after HTML conversion."
  (concat (org-jekyll--yaml-front-matter info) contents))
;; liquid template に誤解されそうなアレコレを置換
(defun org-jekyll--escape-liquid (contents _backend info)
  "Publish src-block to html, Escape liquid tags, e.g. {%, %}"
  (with-temp-buffer
    (insert (s-replace "{%" "&#123;%" (s-replace "%}" "%&#125;" contents)))
    (set-auto-mode t)
    (buffer-substring-no-properties (point-min) (point-max))))
;;
...
(use-package magit
  :bind (("C-x g" . magit-status))
   ...
   @see [[http://qiita.com/syohex/items/a669b35fbbfcdda0cbf2][git-gutter.elの紹介 - Qiita]]
   #+BEGIN_SRC emacs-lisp
