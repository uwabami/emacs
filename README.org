...
** TODO  [[https://emacs-jp.github.io/packages/package-management/package-el][package.el]] [0/1]
   :PROPERTIES:
...
(package-initialize)
   #+end_src
   - [ ] travis-ci.org での elpa の SSL error の workaround どうしたら良いの?
...
(leaf ido-flex-with-migemo
  ...
  (ido-flex-with-migemo-mode)
   ...
   #+END_SRC
** TODO =ido-vertical-mode=: 候補を縦に並べて表示 [/]
   :PROPERTIES:
...
(leaf eww
  ...
  (add-hook 'eww-mode-hook 'eww-mode-hook--disable-image)
   ...
   - [ ] 背景色の指定
   - [ ] 幅の強制
* TODO =elscreen=: バッファー用 GNU screen もどき [0/2]
  :PROPERTIES:
...
(leaf org-agenda
  ...
  ;; (if (file-directory-p
  ;;      (expand-file-name "Public/cc-env/" (getenv "HOME")))
  ;;     (setq org-agenda-files
  ;;           (append
  ;;            (directory-files-recursively
...
;; (leaf ol
;;   :el-get (ol
;;            :url "https://code.orgmode.org/bzg/org-mode/raw/master/lisp/ol.el"))
;; (leaf ol-wl
;;   :if (or (my:dpkg-status "wl-beta")
;;           (my:dpkg-status "wl"))
;;   :after ol
;;   :el-get (ol-wl
...
(leaf org-gcal
  ...
  `((org-gcal-dir          . ,(expand-file-name
    ...
    (org-gcal-down-days    . 30)  ;; 過去 1 month
    (org-gcal-up-days      . 90)  ;; 未来 3 month
  ...
  (when window-system
    (progn
      ...
      (set-fontset-font nil
                        '(#x2200 . #x22FF)
                        (font-spec :family "Ricty" :height 195))
      ;; Greek
      (set-fontset-font nil
                        '(#x0370 . #x03FF)
                        (font-spec :family "Ricty" :height 195))
...
(defun my:load-side-window-config ()
  ...
  (when window-system
    (progn
      ...
      (set-fontset-font nil
                        '(#x2200 . #x22FF)
                        (font-spec :family "Ricty" :height 150))
      ;; Greek
      (set-fontset-font nil
                        '(#x0370 . #x03FF)
                        (font-spec :family "Ricty" :height 150))
...
(leaf *doom-modeline-settings
  :blackout `((isearch-mode . ,(format " %s" (all-the-icons-faicon "search"))))
  :init
  (leaf doom-modeline
    ;; :disabled t
    :ensure t
