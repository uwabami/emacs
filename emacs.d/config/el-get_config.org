...
* [[http://d.hatena.ne.jp/kiwanami/20110221/1298293727][パスワード管理ツール id-manager.el]] [fn:5] [0/1]
  ...
  #+END_SRC
  欲を言えば, KeePassX と相互運用したい所ですが, どうにかできないかなぁ...
  - [ ] KeePassX に import できる形式に相互変換できるように.
* [[http://www.emacswiki.org/emacs/Edit_with_Emacs][edit-server]]
  Chrome/Chromium で textarea を Emacs で編集する
  #+BEGIN_SRC emacs-lisp
    (when (and
           (executable-find "chromium")
           (file-exists-p (concat user-emacs-directory ".edit-server-use")))
      (el-get 'sync '(edit-server))
      (el-get 'sync '(elscreen-edit-server))
      (edit-server-start))
  #+END_SRC
* [[http://d.hatena.ne.jp/tequilasunset/20110102/p1][その場で色を確認したい - Clipboard]] [fn:7]
  #+BEGIN_SRC emacs-lisp
...
[fn:7] その場で色を確認したい - Clipboard: [[http://d.hatena.ne.jp/tequilasunset/20110102/p1]]
