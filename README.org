...
(use-package nsm
  ...
  (setq nsm-settings-file (concat my:d:tmp "network-settings.data")))
   ...
   他にもイロイロありそう．
** version管理: =vc=
   組み込みの vc-mode は modeline の表示にしか使っていない.
   Git 以外は対応しないことにする
   #+BEGIN_SRC emacs-lisp
(use-package vc
  :config
...
* Project管理: =projectile=
  ...
  最近は ghq で移動する事が増えてきたので, どうしたモンかな.
  #+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
(let* ((state (vc-state (buffer-file-name (current-buffer)))))
  (cond
   ((string-equal state "edited")
    (format "%s" state))
   ))
(insert (format-mode-line '(vc-mode vc-mode)))
#+END_SRC
