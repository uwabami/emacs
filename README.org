...
(setq wl-temporary-file-directory "~/Downloads/")
     ...
     GNOME を使っているのであれば [[https://tracker.debian.org/pkg/eweouz][eweouz]] もおもしろそう．とりあえず読み込んでおく.
     #+BEGIN_SRC emacs-lisp :tangle init-wl.el
...
(use-package w3m
  ...
  #+END_SRC
* 補完: =ido=
  補完を ido に任せているパッケージがちらほらあるので設定しておく．
  find-file は helm-find-file より ido + flx-ido + ido-grid-mode の方が
  好みなんだが，どうしようかなぁ...
** =ido= の設定
   #+BEGIN_SRC emacs-lisp
(use-package ido
...
(use-package flx-ido
  ...
  (setq flx-ido-use-faces t
   ...
   #+END_SRC
** =ido-grid-mode=: 補完候補の grid 表示
   #+BEGIN_SRC emacs-lisp
(use-package ido-grid-mode
  :ensure t
  :config
  (setq ido-grid-mode-max-columns 10
        ido-grid-mode-max-rows 5   ;; ヒョコヒョコと動くのが煩わしいので
        ido-grid-mode-min-rows 5   ;; 高さ 5 行に固定しておく．
        ido-grid-mode-scroll-up    #'ido-grid-mode-previous-row
        ido-grid-mode-scroll-down  #'ido-grid-mode-next-row
        ido-grid-mode-prefix-scrolls t
        ido-grid-mode-start-collapsed nil
        ido-grid-mode-keys nil     ;; PageUp/C-p と PageDonw/C-n が逆の方が好み.
...
(defun ido-recentf-open ()
  ...
  (if (find-file (ido-completing-read "Find recent file: " recentf-list))
   ...
   #+END_SRC
** =ido= 関連の keybind
   =ido-grid-mode= の初期設定が気にくわない(=C-p,C-n= と =<up><down>= は逆の方が好き)なので
   それを設定するのがメイン.
   #+BEGIN_SRC emacs-lisp
(defun my:ido-keybind-setup ()
  "update ido keybind"
  (define-key ido-completion-map (kbd "<prior>") 'ido-grid-mode-previous-page)
  (define-key ido-completion-map (kbd "<next>")  'ido-grid-mode-next-page)
  (define-key ido-completion-map (kbd "<left>")  'ido-grid-mode-left)
  (define-key ido-completion-map (kbd "<right>") 'ido-grid-mode-right)
  (define-key ido-completion-map (kbd "<up>")    'ido-grid-mode-previous)
  (define-key ido-completion-map (kbd "<down>")  'ido-grid-mode-next)
...
;; とりあえず helm-find-files 等を当分使ってみる事に．
...
* 補完: =helm=
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package helm
      ...
      (setq helm-autoresize-min-height 40 ; fixed 40%
...
(use-package org-id
  ...
  :init
  (setq org-id-location-files (concat my:d:tmp "org-id-locations"))
  :config
...
(use-package org-agenda
  ...
  :config
  ;; 使用するファイル
  (setq org-agenda-files '("~/Public/cc-env"))
  (dolist (file
           '("Archive.org"
             "Holidays.org"
             "Memo.org"
...
(defadvice skk-setup-modeline (around my:disable-skk-setup-modeline activate)
  ...
  あと，separater は環境によってズレるので，半角スペースに変更
  #+BEGIN_SRC emacs-lisp
