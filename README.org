...
(leaf ivy
  ...
  (setq ivy-use-virtual-buffers t
        ...
        ;; ivy-sort-functions-alist <-- 入力前のリストの並び
...
(leaf all-the-icons-ivy
  ...
  (all-the-icons-ivy-setup)
   ...
   #+END_SRC
** =counsel=: =ivy-read= を使った既存のコマンド群の置き換え
   まだ調整が必要そう．
   #+BEGIN_SRC emacs-lisp
(leaf smex
  :ensure t
...
** =recentf=: 最近使ったファイル履歴の保管
   ファイルを開く際には =counsel-recentf-open= を使うので,
...
(leaf server
  ...
  (unless (server-running-p)
   ...
   #+end_src
* =ibuffer=: buffer の操作
  ibuffer というより hydra の設定の様な気がする．
  #+begin_src emacs-lisp
(leaf ibuffer
...
* チラ見: =quick-preview.el=
  ターミナルの場合でも強制的に動くようにした folk 版を作っている.
  #+begin_src emacs-lisp
(leaf quick-preview
...
(custom-set-faces
 ...
 ;;
 ;; rainbow-delimiter
 ;; '(rainbow-delimiters-unmatched-face   ((t (:foreground "#FFBFFF" :bold t))))
 '(rainbow-delimiters-mismatched-face  ((t (:foreground "#FFBFFF" :bold t))))
 ;;
 '(rainbow-delimiters-depth-1-face     ((t (:foreground "#FF7F7F"))))
 '(rainbow-delimiters-depth-2-face     ((t (:foreground "#7FFF7F"))))
 '(rainbow-delimiters-depth-3-face     ((t (:foreground "#7F7FFF"))))
 '(rainbow-delimiters-depth-4-face     ((t (:foreground "#FFFF7F"))))
 '(rainbow-delimiters-depth-5-face     ((t (:foreground "#7FFFFF"))))
 '(rainbow-delimiters-depth-6-face     ((t (:foreground "#FFBF7F"))))
 '(rainbow-delimiters-depth-7-face     ((t (:foreground "#BFFF7F"))))
 '(rainbow-delimiters-depth-8-face     ((t (:foreground "#7FBFFF"))))
 '(rainbow-delimiters-depth-9-face     ((t (:foreground "#FFFFBF"))))
 ;; moinmoin
 ...
 '(company-scrollbar-bg                ((t (:background "#4c4c4c"))))
  ...
  #+end_src
** モードラインのカスタマイズ: =doom-modeline=
   細かい所弄ってる．
   - アイコン表示の際に(EAWのせいで)幅が狂うので，
     doom-modeline内の幅の勘定をしている関数をoverride
   - SKKの表示が欲しいので追加
   - bufferのアイコンはfilenameじゃなくてmajor-modeを見にいく様にしてみた．
   他にも弄りたい所はあるけれど．
   #+BEGIN_SRC emacs-lisp
(leaf *doom-modeline-setup
  :init
  (leaf doom-modeline
    :ensure t
    ...
    ;;
    (defvar my:skk-status "--[--]")
    (eval-when-compile (require 'doom-modeline-core))
    (doom-modeline-def-segment my:skk-status
      "Show SKK status."
      (propertize
    ...
    (doom-modeline-def-modeline 'custom
      '(" " my:skk-status buffer-info buffer-position matches misc-info remote-host parrot selection-info)
      '(vcs checker minor-modes buffer-encoding process " ")
      )
