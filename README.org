...
(use-package elfeed
  ...
  (with-eval-after-load 'elfeed
    ...
    (load (concat my:d:password-store "/elfeed.gpg")))
  (if (not (file-directory-p (concat (getenv "HOME") "/.cache/elfeed")))
      (make-directory (concat (getenv "HOME") "/.cache/elfeed"))
    )
  (setq elfeed-use-curl t
        elfeed-db-directory (concat (getenv "HOME") "/.cache/elfeed")
        )
