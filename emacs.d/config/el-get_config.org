...
* cl-lib
  Emacs <= 23 では cl-lib を install しておかないとイロイロ嵌ります.
...
* [[http://d.hatena.ne.jp/tequilasunset/20110102/p1][その場で色を確認したい - Clipboard]] [fn:6]
  #+BEGIN_SRC emacs-lisp
    (el-get 'sync '(popup-color-at-point))
    (load (concat el-get-dir "popup-color-at-point/popup-color-at-point.elc"))
  #+END_SRC
* [[https://github.com/tom-tan/auctex-latexmk][auctex-latexmk]] [fn:7]
  ...
  #+END_SRC
* [[https://github.com/vitoshka/zotelo][zotelo.el]] [fn:8]
  文献管理に [[http://www.zotero.org/][Zotero]] を使うようになったので
  #+BEGIN_SRC emacs-lisp
    (when (and (locate-library "tex-buf")
               (executable-find "latexmk"))
      (el-get 'sync '(zotelo)))
...
* tdiary-mode
  tDiary を emacs から編集/更新
  #+BEGIN_SRC emacs-lisp
    (when (file-exists-p (concat user-emacs-directory ".tdiary-use"))
      (el-get 'sync '(tdiary-mode))
      (setq tdiary-init-file "~/Dropbox/tdiary/tdiary-mode.gpg")
...
[fn:1] [[http://shishithefool.blogspot.jp/2012/04/el-get-emacs.html][It's just same as dust ahead of a wind: el-get を使って Emacs でパッケージ管理]]

[fn:2] el-get: [[http://www.emacswiki.org/emacs/el-get]]
...
[fn:6] その場で色を確認したい - Clipboard: [[http://d.hatena.ne.jp/tequilasunset/20110102/p1]]

[fn:7] auctex-latexmk: https://github.com/tom-tan/auctex-latexmk

[fn:8] zotelo.el: [[https://github.com/vitoshka/zotelo]]
