...
** Jekyll 用の HTML を出力する exporter
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     ;; (setq org-jekyll-layout "emacs")
     ;; (setq org-jekyll-categories "[cc-env, emacs]")
     ;; (setq org-jekyll-published "true")
   #+END_SRC
** Project 用の変数定義
   主に出力場所などの設定
   #+BEGIN_SRC emacs-lisp
