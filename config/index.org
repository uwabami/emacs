...
# $Lastupdate: 2012/10/31 23:58:32$
# License: Expat
#
#+TITLE: Emacsの基本設定
...
** OSの違いに起因する条件分岐
   ...
   Unicode の正規化と Mac OS X特有の事情については
...
** [[file:el-get_config.org][El-Get]]
   ...
   #+END_SRC
** [[http://www.kanji.zinbun.kyoto-u.ac.jp/~tomo/elisp/APEL/][APEL]] [fn:9]
   Upstream の Web には
   #+BEGIN_QUOTE
   ...
   #+END_QUOTE
   とある.
   今ん所, 使っている外部emacs-lispでは Wanderlust と howm が依存している.
   #+BEGIN_SRC emacs-lisp
...
*** offlineimap-el
    ...
    #+END_SRC
    設定.
    Wanderlust 使用時には modeline に表示することと, [F1] で offlineimap を起動できるようにした.
    kill するにはどうすれば良いかな...
     #+BEGIN_SRC emacs-lisp
     ...
     #+END_SRC
*** wanderlust
    設定自体は [[file:wl_config.org][Wanderlust の設定]] で行なっているので,     ここでは MUA の設定など
    ...
    設定ファイルの変更: 初期値は =~/.wl=.
       ...
       #+END_SRC
    キーバインド: [F3] で Wanderlust が起動するように
     #+BEGIN_SRC emacs-lisp
...
** [[file:lookup_config.org][lookup-el]]
   CVS 用のレシピは書きたくないので,
   とりあえず =lookup= がインストールされている場合だけ読み込むように.
   #+BEGIN_SRC emacs-lisp
     (if (locate-library "lookup")
...
** [[file:flynamke_config.org][flymakeの設定]]
   ...
   #+END_SRC
** [[http://navi2ch.sourceforge.net/][Navi2ch Project Home Page]]
   #+BEGIN_SRC emacs-lisp
     (my:load-org-file "navi2ch_config.org")
   #+END_SRC
** [[file:lookup_config.org][lookup の設定]]
   #+BEGIN_SRC emacs-lisp
     (my:load-org-file "lookup_config.org")
   #+END_SRC
      ...
      (cond
       ((string-match "\\.\\(?:PDF\\|DVI\\|pdf\\|dvi\\)\\'" filename)
        (let ((process-connection-type nil))
          ;; (start-process "" nil "/usr/bin/kde-open" filename)))
          (start-process "" nil "/usr/bin/kde-open" filename)))
       (t
        ad-do-it)))
  #+END_SRC
