...
;;;###autoload
  ...
  :hook
  (window-setup-hook . my:apply-font-eaw-console-with-emoji)
  )
 #+end_src
 表示のテスト
 #+BEGIN_EXAMPLE
...
(leaf nerd-icons
  ...
  ;; (setq nerd-icons-family "EAW CONSOLE")
  :config
...
** =treesit-fold=: コードの「折り畳み」
   ...
   :END:
   じたばたしながら fortran-parser を追加してみたり.
   #+begin_src emacs-lisp :tangle init.el
(leaf treesit-fold
  :if (executable-find "tree-sitter")
  :ensure t
    ...
    (let ((found-folded nil))
      (save-excursion
        (dolist (ov (overlays-in (point-min) (point-max)))
          ;; 実測値に基づき、invisibleプロパティが 'treesit-fold であるかを確認
      ...
      (when (member type treesit-fold-fortran-declaration-nodes)
        (let ((prev (treesit-fold-range-fortran--sibling node nil))
          ...
          (unless (and prev (member (treesit-node-type prev) treesit-fold-fortran-declaration-nodes))
            (when (and next (member (treesit-node-type next) treesit-fold-fortran-declaration-nodes))
              (let ((last-node next)
                    (iter-node next))
                (while (let ((cand (treesit-fold-range-fortran--sibling iter-node t)))
                         (and cand
                              (member (treesit-node-type cand) treesit-fold-fortran-declaration-nodes)))
