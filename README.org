...
* はじめに
  ...
  基本方針は以下の通り:
** Emacs のパッケージ: [[https://github.com/raxod502/straight.el][straight.el]] と [[https://github.com/jwiegley/use-package][use-package]] を使う
   以前は [[https://github.com/quelpa/quelpa][quelpa]] + [[https://framagit.org/steckerhalter/quelpa-use-package][quelpa-use-package]] を使っていましたが,
   便利なので結局 [[https://github.com/raxod502/straight.el][straight.el]] + [[https://github.com/jwiegley/use-package][use-package]] となりました.
** Debian パッケージがインストールされているならば，それを優先する
   ...
   以下, いたる所で =:straight nil= となっているのはそのためですね.
...
(if (file-directory-p (expand-file-name ".gnupg" (getenv "HOME")))
    (setq package-gnupghome-dir (expand-file-name ".gnupg" (getenv "HOME"))))
(add-to-list 'package-archives '("melpa" . "https://melpa.org/packages/") t)
(add-to-list 'package-archives '("org"   . "https://orgmode.org/elpa/") t)
(eval-when-compile
  (unless (file-exists-p (concat user-emacs-directory "tmp/pkg-stamp"))
    (progn
      (package-refresh-contents)
      (with-temp-buffer (write-file (concat user-emacs-directory "tmp/pkg-stamp")))
      )))
(package-initialize)
   #+END_SRC
   あと batch で先に org-plus-contrib を install する様にしています.
   #+BEGIN_SRC emacs-lisp :tangle org-install.el
(setq url-http-attempt-keepalives nil)
(require 'package nil 'noerror)
(setq package-enable-at-startup t
      package-user-dir (expand-file-name "pkg/elpa" user-emacs-directory)
...
** [[https://framagit.org/steckerhalter/quelpa-use-package][quelpa-use-package]]
   ...
   melpa から導入しておきます.
   とはいえ、[[https://github.com/quelpa/quelpa][quelpa]] はあくまでオプションなので melpa の同期はしません.
   #+BEGIN_SRC emacs-lisp
...
(use-package el-x
  :ensure t)
  #+END_SRC
* [[https://github.com/purcell/exec-path-from-shell][purcell/exec-path-from-shell]]: 環境変数の読み込み
  zsh で設定した =PATH= などの環境変数を Emacs に引き継ぐ.
  今の所
  - =SHELL=
  - =DEBFULLNAME=
  - =DEBEMAIL=
...
(use-package elscreen
  :quelpa (elscreen
            :fetcher url
            :url "https://raw.githubusercontent.com/syohex/elscreen/master/elscreen.el")
  :init
...
(add-to-list 'skk-search-prog-list
    ...
    #+END_SRC
* [[https://github.com/emacs-jp/migemo][migemo]]: インクリメンタル検索 [0/1]
  #+BEGIN_SRC emacs-lisp
(use-package migemo
  ...
  #+END_SRC
  - [ ] ensure したら encoding が原因なのか, install に失敗する模様
* 補完: =ido=
  補完は =ido= が軽いし速いので好み.
...
** メールからの予定の取り込み: =mhc=
   #+BEGIN_SRC emacs-lisp
(use-package mhc
  :if (and (file-directory-p (concat (getenv "HOME") "/.config/mhc"))
           (or (my:dpkg-status "wl")
               (my:dpkg-status "wl-beta")))
  :ensure t
  :config
