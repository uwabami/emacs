...
* インクリメンタルサーチ
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (setq skk-isearch-mode-enable 'never)
...
* 辞書の設定
  ...
  #+BEGIN_SRC emacs-lisp
    ;; 辞書サーバの指定
    (cond
     ((file-exists-p (concat user-emacs-directory ".skkserver-use"))
     ...
     (t
      (setq skk-large-jisyo
            (concat user-emacs-directory "modules/skkdic/SKK-JISYO.L"))
      ))
    ;; 辞書登録の際に送り仮名を削除
