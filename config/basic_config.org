...
* saveplace で前回の修正位置を記憶する.
  ...
  #+END_SRC
* timestamp の自動更新
  ファイル内に =#+DATE: 2= がある場合には,
  save する度に timestamp を更新する. =org-mode= の timestamp の形式に揃えた感じ.
  #+BEGIN_SRC emacs-lisp
    (require 'time-stamp)
...
* emacs-server
  ...
  #+END_SRC
* browse-url
  chrome/chromium → Firefox → emacs-w3m → w3 の順で探索
  #+BEGIN_SRC emacs-lisp
      (require 'browse-url)
      (cond
       ((and
