...
* org-redmine
  #+BEGIN_SRC emacs-lisp
    (require 'org-redmine nil 'noerror)
    (load (expand-file-name "~/ownCloud/org/redmine.gpg"))
    (defun my:org-redmine-select-project()
      (interactive)
      (anything
       `(((name . "Select Project")
          (candidates . ,(mapcar 'car my:org-redmine-project-alist))
          (volatile)
          (action . (lambda (project-name)
                      (progn
                        (setq org-redmine-uri (cadr (assoc project-name my:org-redmine-project-alist)))
                        (setq org-redmine-auth-api-key (caddr (assoc project-name my:org-redmine-project-alist)))
                        (message (format "Redmine Project was changed to \"%s\"." project-name)))))))))
    (setq org-redmine-template-header
          "TODO <%u_date%> %s%")
  #+END_SRC
* MobileOrg
  #+BEGIN_SRC emacs-lisp
    (setq org-mobile-directory (expand-file-name "~/ownCloud/MobileOrg"))
    (setq org-mobile-inbox-for-pull "~/ownCloud/org/mobile.org")
  #+END_SRC
* メモ環境: Org-Capture
   ...
   #+BEGIN_SRC emacs-lisp
     (setq org-capture-templates
           `(
             ...
             ("w" "Wait" entry
              (file (concat org-directory "wait.org"))
              "* WAIT <%<%Y-%m-%d>> %?\n  %i\n  %a\n"
              :prepend nil
              :unnarrowed nil
              :kill-buffer t
