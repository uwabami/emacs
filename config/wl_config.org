...
# $Lastupdate: 2012/09/25 08:02:20$
# License: Expat.
#
...
** フォルダの位置の default からの変更
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (wl-addrbook-setup)
     (define-key wl-summary-mode-map "@" 'wl-summary-addrbook-add)
...
** c-sig
  ...
  一時配布元が良くわからないので
  alioth の Git リポジトリを参照する el-get 用の recipe を書いてみたり.
    #+BEGIN_SRC emacs-lisp
...
* GPG 署名
  ...
  #+END_SRC
* 検索
  offlineimap なんかで local の maildir へメールを同期させている場合は
  ...
  #+END_SRC
  maildir-utils を使う場合には archive の設定はしない方が良い
  (検索できなくなる).
  ...
  ちなみに, これを使わずに =V= での検索で十分, という話もある
* Footnotes
