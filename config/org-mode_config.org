...
# $Lastupdate: 2013/01/28 03:49:29$
# License: Expat
#
#+TITLE: org-modeの設定
...
* org-agenda で GTD
** Agenda で表示するファイル
   =org-directory= 以下の全てのファイル
   #+BEGIN_SRC emacs-lisp
     (setq org-agenda-files
           '("~/Dropbox/org/schedule.org"))
             ;; (expand-file-name (concat org-directory "schedule.org"))
             ;; ))
   #+END_SRC
** Agenda表示で下線を使う
   #+BEGIN_SRC emacs-lisp
   ...
   #+BEGIN_SRC emacs-lisp
     (setq org-refile-targets
           '(
             ...
             ((concat org-directory "schedule.org") :level .1)
             ))
   #+END_SRC
* メモ取り: org-capture
