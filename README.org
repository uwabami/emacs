...
(defun ido-recentf-open ()
  ...
  (interactive)
  (eval-and-compile (require 'recentf nil 'noerror))
...
(leaf id-manager
  :disabled t
  ;; (not (file-exists-p "~/.gnupg/idm-db.gpg"))
  :ensure t
...
** メールからの予定の取り込み: =mhc=
   #+BEGIN_SRC emacs-lisp
(leaf mhc
  :disabled (not (file-directory-p (concat (getenv "HOME") "/.config/mhc")))
  :ensure t
  :config
...
(leaf orgmine
  :if (and my:d:password-store
           (file-directory-p my:d:org))
  :init
  (unless (package-installed-p 'elmine)
