...
(defun my:setup-gui-frame (&optional frame)
  ...
  (let ((f (or frame (selected-frame))))
    (when (display-graphic-p f)
      ...
      (set-face-attribute 'default nil :family "Sarasa Term J" :weight 'light :height 130)
      ;; フォントセットの設定. 対象フレーム (第4引数) を指定せずグローバルに適用する.
      (set-fontset-font nil 'unicode (font-spec :family "Sarasa Term J" :weight 'light))
      ...
      ;;   (set-fontset-font nil code (font-spec :family "Rounded-X Mgen+ 2m") nil 'append)
      ;;   )
      ;; (eval-when-compile (require 'eaw-custom nil :noerror))
      ;; (dolist (target code-wide)
      ;;   (set-fontset-font nil target (font-spec :family "Rounded-X Mgen+ 2m") nil 'append)
      ;;   )
      ;; (dolist (target code-half)
      ;;   (set-fontset-font nil target (font-spec :family "Iosevka Term"))
