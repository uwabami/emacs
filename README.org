...
* はじめに
  ...
  #+html: <img src="https://github.com/uwabami/emacs/actions/workflows/build.yml/badge.svg" width="104px" height="20px" class="github_badge"/>
  #+html: <img src="https://img.shields.io/badge/License-GPLv3-blue.svg" width="92px" height="20px"/>
...
** TODO [[https://github.com/conao3/leaf.el][leaf.el]] [1/2]
   ...
   - [X] =package-vc-install= でインストールした諸々って, package-upgrade 後に delete 対象になる?
     → send PR: [[https://github.com/conao3/leaf.el/pull/552][leaf-handler-vc: Add package to package-selected-packages by uwabami · Pull Request #552 · conao3/leaf.el]]
   #+begin_src emacs-lisp :tangle init.el
;; ---------------------------------------------------------------------------
(eval-and-compile
...
;; ---------------------------------------------------------------------------
;;;###autoload
(defun my:vc-installed-package (pkgname)
  ":vcキーワードでインストールしたパッケージを package-selected-package に登録"
  (leaf-safe-push `,pkgname package-selected-packages 'no-dup))
;; ---------------------------------------------------------------------------
(leaf leaf-keywords
  :ensure t
  :init
  ...
  (leaf-keywords-init)
   ...
   #+end_src
** WAIT =package-vc-install= の調整 [0/1]
   :PROPERTIES:
...
(leaf eaw-custom
  ...
  :preface
  (add-to-list 'package-vc-allow-build-commands 'eaw-custom)
  (my:vc-installed-package 'eaw-custom)
  :init
...
(leaf saveplace
  ...
  (emacs-startup-hook . save-place-mode)
   ...
   #+end_src
   - [ ] org-mode で fold されている場所に対して利用できるのか?
   - [ ] 秘匿すべきファイルは追加した方が良いな.
...
* =ibuffer=: buffer の操作
  ...
  → 最近は [[#orged5e5627][=consult=: 便利コマンド集]] の =consult-buffer= を使う様になってきた.
  あまり呼び出さなくなったけど, 設定は残しておくことに.
  #+begin_src emacs-lisp :tangle init.el
(leaf ibuffer
  :tag "builtin"
  :preface
...
(leaf consult-ghq
  ...
  :preface (my:vc-installed-package 'consult-ghq)
...
(leaf eglot-booster
  ...
  :preface (my:vc-installed-package 'eglot-booster)
  :after eglot
...
(leaf f90format
  ...
  :preface
  (my:vc-installed-package 'f90format)
  :config
...
(leaf f90-ts-mode
  ...
  :preface (my:vc-installed-package 'f90-ts-mode)
...
(leaf project
  ...
  (setq project-list-file (expand-file-name "projects" my:d:tmp)
   ...
   - [ ] 何故か↑の =project-list-file= が効かずに =~/.emacs.d/project= が生成される事が.
     - これはなんだろうか?
** TODO =magit=: Emacs の Git Frontend [0/0]
   :PROPERTIES:
...
(leaf git-commit-ts-mode
  ...
  :preface (my:vc-installed-package 'git-commit-ts-mode)
  :mode "\\COMMIT_EDITMSG\\'"
  )
   #+end_src
* 翻訳
...
(leaf text-adjust
  ...
  :preface (my:vc-installed-package 'text-adjust)
  )
   #+end_src
...
(leaf org-agenda
  ...
  (defvar my:org-agenda-highlight-rules
    '(("Univ:" . my:org-agenda-calendar-Univ)
      ...
      ("mstdn-\*" . my:org-agenda-calendar-SNS)
      ))
...
(leaf orgmine
  ...
  :vc (:url "git@github.com:uwabami/orgmine")
  :preface (my:vc-installed-package 'orgmine)
...
(leaf org-wiki
  ...
  :preface (my:vc-installed-package 'org-wiki)
...
(leaf ox-jekyll
  ...
  :preface (my:vc-installed-package 'ox-jekyll)
  )
(leaf ox-html
  :after ox
  :init
...
(leaf tdiary-mode
  ...
  :preface (my:vc-installed-package 'tdiary-mode)
  :config
