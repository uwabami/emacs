...
** East Asian Ambiguos 対応
   ...
   アイコンフォントとして  [[https://github.com/sebastiencs/icons-in-terminal][icons in terminal]] にも対応しています.
   #+begin_src emacs-lisp
(use-package locale-eaw-emoji
  ...
  (eaw-and-emoji-fullwidth))
   #+end_src
   本当は all the icons に対応した方が良いのかもしれないけれど,
   Surrogate, Variation Selector と Private Use Area の区別がついてない
   アイコンフォントが多すぎて, 割とシンドイなぁ, とか思ったり.
** icon font
   all-the-icons のデータを修正して,
   icons in terminal のデータを読みに行くようにしてみたり.
   - フォントは [[https://github.com/uwabami/icons-in-terminal][github.com/uwabami/icons-in-terminal]]
   - 修正版は [[https://github.com/uwabami/all-the-icons-in-terminal][all-the-icons-in-terminal]]
   元ファイルと変えていないので, 実際はデータのみ提供するようにした方が無難, かも.
   #+BEGIN_SRC emacs-lisp
(use-package all-the-icons
  :ensure t
  )
(use-package all-the-icons-in-terminal
  :quelpa (all-the-icons-in-terminal
           :fetcher github
           :repo "uwabami/all-the-icons-in-terminal"
           :files (:defaults "data"))
  :config
  (with-eval-after-load 'all-the-icons-in-terminal
    (add-to-list 'all-the-icons-mode-icon-alist
                 '(f90-mode all-the-icons-faicon "facebook" :height 1.0)) ; facebook!
    (add-to-list 'all-the-icons-mode-icon-alist
                 '(wl-folder-mode all-the-icons-faicon "folder-o" :height 1.0))
    (add-to-list 'all-the-icons-mode-icon-alist
                 '(wl-summary-mode all-the-icons-faicon "folder-open-o" :height 1.0))
    (add-to-list 'all-the-icons-mode-icon-alist
                 '(wl-draft-mode all-the-icons-material "drafts" :height 1.0))
    (add-to-list 'all-the-icons-mode-icon-alist
                 '(mime-view-mode all-the-icons-faicon "envelope-o" :height 1.0))
    )
  )
   #+END_SRC
** OSの違いに起因する条件分岐
...
(use-package undo-tree
  :ensure t
  :init
  ;; @see https://github.com/raxod502/radian/commit/0544196c50ddc305f90873ca280bf0c5e3db685b
  ...
  (advice-add #'undo-tree-load-history :around
              #'my:undo-tree-suppress-buffer-modified-message)
  :config
...
(use-package quick-preview
  :quelpa (quick-preview
           :fetcher github
           :repo "uwabami/quick-preview.el")
  :config
...
(use-package projectile
  ...
  (setq projectile-project-search-path nil
        ...
        projectile-mode-line-function #'(lambda () projectile-mode-line-prefix)
        )
...
* MoinMoin: =moinmoin-mode=, =moomin.el=
  ...
  MoinMoin の設定. 編集には [[https://github.com/lurdan/moomin-el][tototoshi/moomin-el: Edit MoinMoin with Emacs]] を使う.
** =moinmoin-mode=
...
(use-package moinmoin-mode
  :quelpa (moinmoin-mode
   ...
   #+END_SRC
   そのうち気になる所を弄る，ということで．
** =moomin=: =emacs-request + helm + moinmoin-mode=
   最新の奴だと動かなかったので folk して多少弄った版を使うことに.
...
(use-package moomin
  ...
  (setq moomin-history-file (concat my:d:tmp "moomin_history"))
  )
#+END_SRC
* 日記: =tDiary=
...
** major/minor モードの表示設定
   ...
   [[https://github.com/sebastiencs/icons-in-terminal][icons in terminal]] を使っているので, 対応するフォントが必要.
   #+BEGIN_SRC emacs-lisp
