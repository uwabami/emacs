...
** TODO [[https://emacs-jp.github.io/packages/package-management/package-el][package.el]] の初期化処理 [0/1]
   ...
   #+begin_src emacs-lisp :tangle init.el
;; ---------------------------------------------------------------------------
(defvar my:package-menu-exclude-packages nil
  ...
  (let ((included (-filter
                   (lambda (entry)
                     (let ((name (symbol-name (package-desc-name (car entry)))))
                       (not (member name my:package-menu-exclude-packages))))
...
(leaf migemo
  ...
  :ensure t
  :preface
  (add-to-list 'my:package-menu-exclude-packages "migemo")
  :init
...
(leaf cape
  ...
  :ensure t
  :advice
  (:around eglot-completion-at-point
           cape-wrap-buster)
  :init
  :config
...
(leaf consult
  ...
  :config
;;  (setq read-file-name-function #'my:consult-find-file-with-preview
;;        consult-preview-excluded-buffers
;;        consult--source-buffer
;;        (plist-put consult--source-buffer :state #'my:consult-buffer-state-no-tramp))
  ;; -------------------------------------------------------------------------
;;  (defun my:consult-find-file-with-preview (prompt &optional dir default mustmatch initial pred)
;;    (interactive)
...
;;                     :predicate pred)))
  ;; -------------------------------------------------------------------------
  (defun my:consult-buffer-state-no-tramp ()
...
(leaf flymake
  ...
  :config (require 'flymake-proc nil 'noerror)
  )
...
(leaf c-sig
  ...
  :preface
  (add-to-list 'my:package-menu-exclude-packages "c-sig")
  :init
...
(defun my:org-compat-adjust-tab-width-in-buffer (old-width)
  ...
  (leaf howm
    :ensure t
    :preface
    (add-to-list 'my:package-menu-exclude-packages "howm")
    )
