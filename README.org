...
(use-package browse-url
  ...
  ;; default は eww. eww から firefox を呼び出すようにする
...
(use-package server
  ...
  (unless (server-running-p)
   ...
   #+END_SRC
* 日本語入力関連.
  以前は ddskk を使っていたのだけれど,
  Emacs は "-nox", つまり frame 無しでターミナル上で上げることが多いので
  XIM 経由で入力することに.
  #+BEGIN_SRC emacs-lisp
(defun my:xim-message ()
  (interactive)
  (message "Check your IM status"))
(bind-key  "C-\\" 'my:xim-message)
  #+END_SRC
  ...uim をコマンドラインから有効にする方法は無いものかなぁ...
* 日本語入力: =ddskk=
  ここは *obsolete* . 最近は xim 経由で入力している.
...
*** 個人情報の設定
    具体的な設定内容は以下のファイルに置いている
     #+BEGIN_SRC emacs-lisp :tangle init-wl.el
     (load (concat my:d:password-store "/emacs/wl-info-" system-name ".gpg"))
     #+END_SRC
    設定している内容は以下の通り
...
(setq wl-queue-folder "+queue")
     ...
     folders の位置の変更
     #+BEGIN_SRC emacs-lisp :tangle init-wl.el
(setq wl-folders-file (concat my:d:password-store "/emacs/wl-folders-" system-name ".gpg"))
     #+END_SRC
     Drafts, Trash の置き場所
     #+BEGIN_SRC emacs-lisp :tangle init-wl.el
...
(custom-set-faces
 ...
 '(avy-lead-face                   ((t (:foreground "#020202" :background "#244424" :bold t))))
 '(avy-lead-face-0                 ((t (:foreground "#020202" :background "#444424" :bold t))))
 '(avy-lead-face-1                 ((t (:foreground "#020202" :background "#442444" :bold t))))
 '(avy-lead-face-2                 ((t (:foreground "#020202" :background "#244444" :bold t))))
 )
  #+END_SRC
* 日記: =tDiary=
...
(use-package tex-jp
  ...
  (setq TeX-source-correlate-method 'synctex
        ...
        '((output-dvi "xdvi")        ;; built-in
          (output-pdf "Zathura")     ;; built-in
          (output-html "xdg-open"))  ;; built-in
