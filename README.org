...
(use-package browse-url
  ...
  (setq browse-url-browser-function 'browse-url-firefox)
   ...
   #+END_SRC
   内蔵ブラウザである eww の設定もついでにやっておく．
   検索エンジンとして =google= を指定したら，403が返ってきた．なんだろうね.
   #+BEGIN_SRC emacs-lisp
(use-package eww
  :config
...
(use-package org-id
  ...
  (setq org-id-link-to-org-use-id 'create-if-interactive-and-no-custom-id
        org-id-location-files (concat my:d:tmp "org-id-locations")
        )
...
(use-package tdiary-mode
  ...
  (setq tdiary-browser-function 'browser-url
        tdiary-coding-system 'utf-8-unix
