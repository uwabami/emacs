...
# $Lastupdate: 2012/03/28 04:59:20$
# License: Expat.
#
#+TITLE: Emacsの基本設定
...
*** バックアップとauto-saveの作成
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (setq auto-save-list-file-prefix
            (concat my:emacs-backup-dir ".saves-"))
...
** [[http://www.kanji.zinbun.kyoto-u.ac.jp/~tomo/elisp/APEL/][APEL]] [fn:8] に load-path を通す
   Web には
   #+BEGIN_QUOTE
   ...
   後者は無いと途方に暮れてしまうので, ここでload-pathの設定をしておく.
   #+BEGIN_SRC emacs-lisp
   ...
   [[file:ddskk.org][DDSKKの設定]] で行なっている.
    #+BEGIN_SRC emacs-lisp
      ...
      (setq skk-user-directory
            (expand-file-name (concat user-emacs-directory "etc/skk")))
      ;; ただし設定ファイルだけは ~/.emacs.d/config 以下に置く
      (setq skk-init-file
            (expand-file-name (concat user-emacs-directory "config/ddskk")))
      ;; org-babel-tangle で ddskk.org から ddskk.el を生成
      (my:org-babel-tangle-and-compile-file
       (expand-file-name (concat user-emacs-directory "config/ddskk.org")))
...
** MUA
   ...
   #+BEGIN_SRC emacs-lisp
     (when (locate-library "wl")
       (my:load-org-file "extra/wl_config.org"))
   #+END_SRC
...
*** install
    ...
    適宜修正しておくこと.
*** 設定
    リポジトリとして [[http://marmalade-repo.org/][Marmalade]] [fn:11]も追加しておく
    #+BEGIN_SRC emacs-lisp
      (require 'package)
...
[fn:11] Marmalade: [[http://marmalade-repo.org/]]

[fn:10] align設定用例集: [[http://handlename.hatenablog.jp/entry/2011/12/11/214923]]
