...
* 補完: =helm=
  ...
  find-file の挙動だけはどうしても好きになれない。
  補完が途中で止まって欲しいんだがなぁ(もしくは TAB 補完使いたい、とも言う)。
  #+BEGIN_SRC emacs-lisp
(use-package helm
  :ensure t
  :defines helm-map
...
(use-package elscreen
  ...
  (elscreen-start))
  #+END_SRC
* [[http://syohex.hatenablog.com/entry/20130914/1379166283][zoom-window]]
  #+BEGIN_SRC emacs-lisp
...
(use-package git-gutter
  ...
  (global-git-gutter-mode +1)
   ...
   #+END_SRC
** VCS: =helm-ghq=
   ghq があった時に helm でリストアップする。
   フルパス表示がちょっとうざったいので、関数群をちょっと override することに。
   #+BEGIN_SRC emacs-lisp
(use-package helm-ghq
  ...
  :bind (("C-x f" . helm-ghq))
  :config
  (defun my:helm-ghq--list-candidates ()
    (with-temp-buffer
      ...
      (let ((ghq-root (helm-ghq--root))
        ...
        (while (not (eobp))
          ...
          (forward-line 1))
        (reverse paths))))
  (advice-add 'helm-ghq--list-candidates :override
              #'my:helm-ghq--list-candidates)
  )
   #+END_SRC
** 設定ファイル: =generic-x=
   #+BEGIN_SRC emacs-lisp
...
(use-package auto-complete-config
  ...
  ;;  (add-to-list 'ac-modes mode))
   ...
   #+END_SRC
   org-mode には org-ac を使うことに: [[https://github.com/aki2o/org-ac][gitub: aki2o/org-ac]]
   #+BEGIN_SRC emacs-lisp
(use-package org-ac
  :ensure t
  :config
...
(custom-set-faces
 ...
 '(helm-selection                      ((t (:inherit highlight )))) ;; background "#4c4c4c" ))))
